...
class InvariantMarketNameTranslationIT {
    ...
    public InvariantMarketNameTranslationIT() throws Exception {}
    ...
    void validatesMarketAndOutcomeNamesInReceivedOddsChangeMessage(Locale language) throws Exception {
        ...
        apiSimulator.stubMarketListContaining(oddEvenMarketDescription(language), language);

        LogsMock apiCallLogs = LogsMock.createCapturingFor(LogHttpDataFetcher.class);
        ...
        try (
            ...
            assertThat(market.getName(language)).isEqualTo(oddEvenMarketDescription(language).getName());
            OutcomesAssert
                .assertThat(market.getOutcomeOdds())
                .hasExactlyNamesAndIdsEqualTo(oddEvenMarketDescription(language));
            apiCallLogs.verifyLoggedLineContaining(format("/v1/descriptions/%s/markets.xml", language));
        }
    }
