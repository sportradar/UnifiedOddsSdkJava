    ...
    class Holder {
        ...
        private void sendMessageAt(TimeUtils timeUtils) {
            doAnswer(invocationOnMock -> {
                ...
                verify(channel).basicConsume(anyString(), anyBoolean(), anyString(), messageConsumer.capture());
                when(timeUtils.now()).thenReturn(invocationOnMock.getArgument(0));
                messageConsumer.getValue().handleDelivery("any", mock(Envelope.class), mock(AMQP.BasicProperties.class), new byte[]{});
                return null;
            }).when(channel).sendMessageAt(anyLong());
        }

        private void verifyInitiated() {
            doAnswer(invocationOnMock -> {
                verify(channel, invocationOnMock.getArgument(0)).basicConsume(any(), anyBoolean(), anyString(), any());
                return null;
            }).when(channel).verifyInitiated(any());
        }
