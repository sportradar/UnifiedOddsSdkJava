...
public class ConfigurationBuilderBaseTests {
    ...
    public void testSetValues() {
        ...
        Assert.assertEquals(
            builderWithEmptyProperties.exceptionHandlingStrategy,
    ...
    public void testReadValuesFromProperties() {
        ...
        Assert.assertNotEquals(
            builderWithFullProperties.defaultLocale,
            SDKPropertiesReaderUtil.DEFAULT_LOCALE
        );
        Assert.assertNotEquals(
            builderWithFullProperties.exceptionHandlingStrategy,
            SDKPropertiesReaderUtil.EXCEPTION_HANDLING
        );
        Assert.assertFalse(
            builderWithFullProperties.nodeId != null &&
            builderWithFullProperties.nodeId == SDKPropertiesReaderUtil.SDK_NODE_ID
        );
        ...
        Assert.assertTrue(
            builderWithFullProperties.getSupportedLocales().size() ==
            SDKPropertiesReaderUtil.DESIRED_LOCALES.size()
        );
        Assert.assertTrue(
            SDKPropertiesReaderUtil.DESIRED_LOCALES.containsAll(
                builderWithFullProperties.getSupportedLocales()
            )
        );

        Assert.assertTrue(
            SDKPropertiesReaderUtil.DISABLED_PRODUCERS.size() ==
            builderWithFullProperties.disabledProducers.size()
        );
        Assert.assertTrue(
            SDKPropertiesReaderUtil.DISABLED_PRODUCERS.containsAll(
                builderWithFullProperties.disabledProducers
            )
        );
    }
    ...
    private static TestableConfigurationBuilderBase getBuilderWithFullProperties() {
        return new TestableConfigurationBuilderBase(
    ...
    private static TestableConfigurationBuilderBase getBuilderWithMinProperties() {
        return new TestableConfigurationBuilderBase(
            SDKPropertiesReaderUtil.getReaderWithMinData(),
            SDKPropertiesReaderUtil.getYamlReaderWithMinData()
        );
    }

    private static class TestableConfigurationBuilderBase extends ConfigurationBuilderBaseImpl {

        TestableConfigurationBuilderBase(
