...
public class CustomBetEntityTests {
    ...
    {
        CAPIAvailableSelections availableSelectionsType = RestMessageBuilder.getAvailableSelections(URN.parse("sr:match:1000"), 10);
        AvailableSelections resultAvailableSelections = SdkMessageBuilder.getAvailableSelections(availableSelectionsType);
    ...
    {
        CAPIAvailableSelections availableSelectionsType = RestMessageBuilder.getAvailableSelections(URN.parse("sr:match:1000"), 0);
    ...
    public void CalculationEmptyMapTest() throws ParseException {
        CAPICalculationResponse calculationResponseType = RestMessageBuilder.getCalculationResponse(URN.parse("sr:match:1000"), 0);
        ...
        Assert.assertEquals(0, Double.compare(calculationResponseType.getCalculation().getOdds(), calculation.getOdds()));
        Assert.assertEquals(0, Double.compare(calculationResponseType.getCalculation().getProbability(), calculation.getProbability()));
        ...
        {
            Assert.assertEquals(SdkHelper.toDate(calculationResponseType.getGeneratedAt()), calculation.getGeneratedAt());
        }
    }

    @Test
    public void CalculationMapTest() throws ParseException {
        CAPICalculationResponse calculationResponseType = RestMessageBuilder.getCalculationResponse(URN.parse("sr:match:1000"), 7);
        Calculation calculation = SdkMessageBuilder.getCalculation(calculationResponseType);
        ...
        Assert.assertEquals(calculationResponseType.getAvailableSelections().getEvents().size(), calculation.getAvailableSelections().size());
        for (int i = 0; i < calculationResponseType.getAvailableSelections().getEvents().size(); i++)
        {
            CAPIEventType sourceAvailableSelection = calculationResponseType.getAvailableSelections().getEvents().get(i);
            AvailableSelections resultAvailableSelection = calculation.getAvailableSelections().get(i);

            Assert.assertEquals(URN.parse(sourceAvailableSelection.getId()), resultAvailableSelection.getEvent());
            for (CAPIMarketType sourceMarket : sourceAvailableSelection.getMarkets().getMarkets())
            {
                Market resultMarket = resultAvailableSelection.getMarkets().stream().filter(f -> f.getId() == sourceMarket.getId()).findFirst().get();
                Assert.assertNotNull(resultMarket);
                MarketCompare(sourceMarket, resultMarket);
            }
        }
        int marketCount = calculation.getAvailableSelections().stream().flatMap(s -> s.getMarkets().stream()).collect(Collectors.toList()).size();
        int outcomeCount = calculation.getAvailableSelections().stream().flatMap(s -> s.getMarkets().stream().flatMap(f2->f2.getOutcomes().stream())).collect(Collectors.toList()).size();
        System.out.println(String.format("Calculation has {} markets and {} outcomes.", marketCount, outcomeCount));
    }


    @Test
    public void CalculationFilterEmptyMapTest() throws ParseException {
        CAPIFilteredCalculationResponse calculationResponseType = RestMessageBuilder.getFilteredCalculationResponse(URN.parse("sr:match:1000"), 0);
    ...
    public void CalculationFilterMapTest() throws ParseException {
        ...
        for (int i = 0; i < calculationResponseType.getAvailableSelections().getEvents().size(); i++)
        {
            CAPIFilteredEventType sourceAvailableSelection = calculationResponseType.getAvailableSelections().getEvents().get(i);
            ...
            {
                MarketFilter resultMarket = resultAvailableSelection.getMarkets().stream().filter(f -> f.getId() == sourceMarket.getId()).findFirst().get();
                Assert.assertNotNull(resultMarket);
                MarketCompare(sourceMarket, resultMarket);
            }
        }
    ...
    public void GetCalculationTest() throws CommunicationException {
        ...
        Selection selection = customBetSelectionBuilder.setEventId(eventId).setMarketId(market.getId()).setOutcomeId(market.getOutcomes().get(0)).setSpecifiers(market.getSpecifiers()).build();
        matchSelections.add(selection);
        market = availableSelections.getMarkets().get(availableSelections.getMarkets().size()-1);
        selection = customBetSelectionBuilder.setEventId(eventId).setMarketId(market.getId()).setOutcomeId(market.getOutcomes().get(0)).setSpecifiers(market.getSpecifiers()).build();
        matchSelections.add(selection);
    ...
    private void AvailableSelectionsCompare(CAPIAvailableSelections source, AvailableSelections result)
    {
        ...
        {
            Market resultMarket = result.getMarkets().stream().filter(f -> f.getId() == sourceMarket.getId()).findFirst().get();
            Assert.assertNotNull(resultMarket);
            MarketCompare(sourceMarket, resultMarket);
        }
    }
    ...
    private void MarketCompare(CAPIFilteredMarketType source, MarketFilter result)
    {
        ...
        for (CAPIFilteredOutcomeType sourceOutcome : source.getOutcomes()) {
            OutcomeFilter resultOutcome = result.getOutcomes().stream().filter(f -> f.getId() == sourceOutcome.getId()).findFirst().get();
