...
import static com.sportradar.unifiedodds.sdk.conn.WaitingOddsFeedListenerDi.createWaitingOddsFeedListenerFactory;
...
public class SdkConnectionTests {
    ...
    public SdkConnectionTests() throws Exception {}
    ...
    public void connectionBreakMultiSessionTest() throws InitException {
        ...
        assertEquals(
            ...
            sdkListener.CalledEvents
                .stream()
                .filter(c -> c.contains("Producer LO is down: ConnectionDown"))
                .collect(Collectors.toList())
                .size()
        ); // producers are marked down
        assertEquals(
            ...
            sdkListener.CalledEvents
                .stream()
                .filter(c -> c.contains("Producer Ctrl is down: ConnectionDown"))
                .collect(Collectors.toList())
                .size()
        );
        assertEquals(
            ...
            sdkListener.CalledEvents
                .stream()
                .filter(c -> c.contains("Producer VF is down: ConnectionDown"))
                .collect(Collectors.toList())
                .size()
        );
        Helper.sleep(2000);
        ...
        waitAndCheckTillTimeout(
            w -> checkListContainsString(sdkListener.CalledEvents, "Connection to the feed lost", 1),
            "Connection to the feed lost",
            1000,
            60000
        );
    ...
    public void noApiCallsShouldOccurOnFeedReceivingOddsChangeWithMarketHavingNonVariantDescription()
        throws Exception {
        val listener = createWaitingOddsFeedListenerFactory().expectingOddsChange();
        feed
            .getSessionBuilder()
    ...
    private void waitAndCheckTillTimeout(
     ...
     * @return true if isFlaggedDown()==isDown, else false
     */
    private boolean checkListContainsString(
        List<String> list,
        String firstText,
        String secondText,
        int count
