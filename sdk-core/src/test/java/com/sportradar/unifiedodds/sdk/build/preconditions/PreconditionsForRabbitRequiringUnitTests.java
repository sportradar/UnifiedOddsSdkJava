...
public class PreconditionsForRabbitRequiringUnitTests {
    ...
    private static void validateRabbitMqIsUp() {
        if (shouldMavenRunUnitTests() && shouldMavenRunTestsExercisingRabbitServer()) {
            if (!isRabbitMqServerUp()) {
                throw new IllegalStateException(
                    ...
                    "and Toxiproxy server interface to be running on: " +
                    TOXIPROXY_BASE_URL.get() +
                    ", however the ports are not opened. " +
                    "Start RabbitMQ And Toxiproxy servers " +
                    "by bringing docker compose up in local-development folder. " +
