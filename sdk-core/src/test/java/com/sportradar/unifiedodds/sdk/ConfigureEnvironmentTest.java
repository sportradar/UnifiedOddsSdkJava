    ...
    public static class EnvironmentUpdater {
        ...
        public void updateToIntegrationEnvironmentFromCustom() {
            UofConfiguration config = builder
                ...
                .setDefaultLanguage(anyLanguage)
                .build();
            com.sportradar.unifiedodds.sdk.cfg.EnvironmentUpdater updater = createUpdaterFrom(config);
            ...
            assertThat(config.getEnvironment()).isEqualTo(Integration);
            assertThat(internalConfig.getEnvironment()).isEqualTo(Integration);
            assertThat(internalConfigForNonReplayExplicitly.getEnvironment()).isEqualTo(Integration);
        }
        ...
        public void updateToProductionEnvironmentFromCustom() {
            ...
            assertThat(internalConfig.getEnvironment()).isEqualTo(Production);
            assertThat(internalConfigForNonReplayExplicitly.getEnvironment()).isEqualTo(Production);
        }

        @Test
        public void sdkInternalConfigurationIsKeptInTheLoopOfEnvironmentUpdates() {
            UofConfiguration config = builder
                .setAccessToken(ANY_TOKEN)
                .selectCustom()
                .setDefaultLanguage(anyLanguage)
                .build();
            com.sportradar.unifiedodds.sdk.cfg.EnvironmentUpdater updater = createUpdaterFrom(config);
            final val internalConfig = new SdkInternalConfiguration(config, anyProps(), anyYaml());
            final val internalConfigForNonReplayExplicitly = new SdkInternalConfiguration(
                config,
                nonReplayMode,
                anyProps(),
                anyYaml()
        ...
        private com.sportradar.unifiedodds.sdk.cfg.EnvironmentUpdater createUpdaterFrom(
            final UofConfiguration configuration
        ) {
            return Guice
                .createInjector(new ConfigurationProvidingModule(configuration))
                .getInstance(com.sportradar.unifiedodds.sdk.cfg.EnvironmentUpdater.class);
        }
