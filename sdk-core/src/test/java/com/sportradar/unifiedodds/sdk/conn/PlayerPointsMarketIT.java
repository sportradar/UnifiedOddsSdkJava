...
import static com.sportradar.unifiedodds.sdk.conn.SapiMarketDescriptions.PlayerPoints.playerPointsInvariantMarketDescription;
import static com.sportradar.unifiedodds.sdk.conn.SapiMarketDescriptions.PlayerPoints.playerPointsVariantMarketDescription;
import static com.sportradar.unifiedodds.sdk.conn.UfMarkets.WithOdds.playerPointsMarket;
...
public class PlayerPointsMarketIT {
    ...
    public PlayerPointsMarketIT() throws Exception {}
    ...
    void fetchesTranslatedPlayerPointsMarketNameAndOutcomeNamesForSingleVariantMarket(Locale language)
        throws Exception {
        ...
        apiSimulator.stubMarketListContaining(playerPointsInvariantMarketDescription(language), language);
        apiSimulator.stubSingleVariantMarket(playerPointsVariantMarketDescription(language), language);
        ...
        try (
            ...
            rabbitProducer.send(messages.oddsChange(playerPointsMarket()), routingKeys.liveOddsChange());

            val market = theOnlyMarketIn(listenerWaitingFor.theOnlyOddsChange());

            assertThat(market.getName(language))
                .isEqualTo(playerPointsVariantMarketDescription(language).getName());
            OutcomesAssert
                .assertThat(market.getOutcomeOdds())
                .hasExactlyNamesAndIdsEqualTo(playerPointsVariantMarketDescription(language));
        }
    }
