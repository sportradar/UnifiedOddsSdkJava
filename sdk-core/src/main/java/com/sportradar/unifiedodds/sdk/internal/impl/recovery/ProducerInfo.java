...
class ProducerInfo {
    ...
    ProducerInfo(
        ...
        registerProducerDownUsageMetric(producerId, producerManager, usageTelemetryFactory);
    }

    private void registerProducerDownUsageMetric(
        int producerId,
        SdkProducerManager producerManager,
        TelemetryFactory usageTelemetryFactory
    ) {
        usageTelemetryFactory.gauge(
            UsageGauge.PRODUCER_STATUS,
            () -> {
                ImmutableMap.Builder<String, String> attributes = getProducerDownUsageMetricAttributes(
                    producerId,
                    producerManager
                );
                if (isFlaggedDown()) {
                    attributes.put("reason", determineProducerDownReasonAttribute());
                }
                return gaugeValue(this.isFlaggedDown() ? 0L : 1L, attributes.build());
            }
        );
    }

    private static ImmutableMap.Builder<String, String> getProducerDownUsageMetricAttributes(
        int producerId,
        SdkProducerManager producerManager
    ) {
        String producer = producerId + "-" + producerManager.getProducer(producerId).getName();
        return ImmutableMap.<String, String>builder().put("producer", producer);
    }

    private String determineProducerDownReasonAttribute() {
        return Optional.ofNullable(producerDownReason).map(ProducerDownReason::name).orElse("unknown");
    }
